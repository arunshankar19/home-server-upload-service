...
func NewUploadHandler(tracer observability.Tracer, uploadUsecase domain.UploadUsecase) domain.UploadHandler {
...
func (h *uploadHandler) InitiateUpload(w http.ResponseWriter, r *http.Request) {
	ctx, span := h.tracer.StartSpan(r.Context(), "Handler-InitiateUpload")
	defer span.End()
...
func (h *uploadHandler) GetPresignedURL(w http.ResponseWriter, r *http.Request) {
	ctx, span := h.tracer.StartSpan(r.Context(), "Handler-GetPresignedURL")
	defer span.End()
	...
	url, err := h.uploadUsecase.GetPresignedURL(ctx, presignedURLReq)
	if err != nil {
		response.Fail(w, http.StatusInternalServerError, http.StatusText(http.StatusInternalServerError))
		return
	}
...
func (h *uploadHandler) CompleteUpload(w http.ResponseWriter, r *http.Request) {
	ctx, span := h.tracer.StartSpan(r.Context(), "Handler-CompleteUpload")
	defer span.End()
	...
	err = h.uploadUsecase.CompleteUpload(ctx, completeUploadReq)
	if err != nil {
		response.Fail(w, http.StatusInternalServerError, http.StatusText(http.StatusInternalServerError))
		return
	}
